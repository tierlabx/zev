...
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@zev/ui/components/card";
import { motion, useSpring, useTransform } from "framer-motion";
import { Activity, Database, ShieldCheck, UserCheck, Users, Cpu, HardDrive } from "lucide-react";
import { useEffect } from "react";
import { getDashboardStats, type DashboardActivityRes } from "@/api/system/dashboard";
								...
								<AreaChart data={trends} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
									<defs>
										<linearGradient id="colorCount" x1="0" y1="0" x2="0" y2="1">
											<stop offset="5%" stopColor="var(--color-primary, #18181b)" stopOpacity={0.3} />
						...
						<CardContent className="space-y-4">
							<div className="space-y-1">
								<div className="flex justify-between text-xs">
									<span className="flex items-center gap-1 text-muted-foreground"><Cpu className="h-3 w-3"/> CPU 使用率</span>
							...
							<div className="space-y-1">
								<div className="flex justify-between text-xs">
									<span className="flex items-center gap-1 text-muted-foreground"><HardDrive className="h-3 w-3"/> 内存使用率</span>
							...
							<div className="space-y-1">
								<div className="flex justify-between text-xs">
									<span className="flex items-center gap-1 text-muted-foreground"><Database className="h-3 w-3"/> DB 连通率</span>
						...
						<CardContent>
							{isLoading ? (
								...
								<div className="space-y-4">
									{activities.map((act: DashboardActivityRes) => (
